  ...
  <div className="border border-gray-200 rounded-2xl p-4 hover:shadow-2xl transition-all duration-300 transform hover:-translate-y-2 bg-white group relative">
    <div onClick={onClick} className="cursor-pointer">
      ...
      <div className="flex items-center gap-2 mb-1">
        <p className="text-brand-500 font-bold text-lg">{convertPrice(product.price)}</p>
      ...
      <div className="mt-3 flex flex-wrap gap-2">
        {product.isBestseller && <span className="inline-block px-3 py-1 text-xs font-semibold bg-gradient-to-r from-brand-300 to-brand-100 text-brand-700 rounded-full">Bestseller</span>}
    ...
    {onToggleWishlist && (
      ...
      >
        <Heart
          className={`w-5 h-5 transition-colors ${isInWishlist ? 'fill-[#A88B6A] text-brand-500' : 'text-gray-400 hover:text-brand-500'}`}
        />
      </button>
    )}
  </div>
